...
<style>
   ...
   #title{font-family:ITC Grouch;}
    #modify{font-family: adobe-caslon-pro, serif; font-weight:bold; background-color: yellow; 
   align:right; border:0; outline:0; padding:10px 30px; font-size: 15px; }
   #delete{font-family: adobe-caslon-pro, serif; font-weight:bold; background-color: black; color:yellow; 
    ...
    <!-- dongso -->
						...
						<table class="table table-borderless">
							<thead>	
    							<tr>
									...
									<th scope="col">작품명</th>
									<th scope="col">작가명</th>
									<th scope="col">총 응찰 인원</th>
									<th scope="col">낙찰가</th>
									<th scope="col" style="color: red">&nbsp; 삭제</th>
									<th scope="col" style="color: red">&nbsp; 낙찰</th>
								</tr>
    						</thead>
							<tbody>
    						<c:choose>
								<c:when test="${empty requestScope.auctionDtoList}">
									<tr>
										<td colspan="12">
											<p align="center">
												<b><span style="font-size: 14pt; font-weight: bold">종료된 경매가 없습니다.</span></b>
											</p>
										</td>
									</tr>
								</c:when>
    							...
    							<c:forEach items="${requestScope.auctionDtoList}" var="Auction">	
										<tr onmouseover="this.style.background='white'"
											...
											<td bgcolor="">
												<p>
													<span style="font-size: 9pt;">
														<form action="${path}/front?key=auction&methodName=deleteAuction" id="AuctionDelete" method="post">
											...
											<td bgcolor="">
												<p>
													<span style="font-size: 9pt;">
															<c:if test="${Auction.aucState == 3}">
																<form action="${path}/front?key=auction&methodName=insertSuccedBid" method="post" id="bidding">
																	<input type="submit" value="낙찰" style="width:-10px;float:left;" class="genric-btn primary"/>
																	<input type="hidden" name="aucCode" value="${Auction.aucCode}">
																	<input type="hidden" name="artCode" value="${Auction.art.artCode}">			
		...
		<c:forEach items="${requestScope.auctionDtoList}" var="Auction">
			...
			<td>
				...
				<c:if test="${Auction.aucState == 3}">
					<form action="${path}/front?key=auction&methodName=insertSuccedBid" method="post" id="bidding">
						<input type="submit" value="낙찰" style="width:-10px;float:left;"/>
						<input type="hidden" name="aucCode" value="${Auction.aucCode}">
						<input type="hidden" name="artCode" value="${Auction.art.artCode}">			
						<input type="hidden" name="state" value="${Auction.aucState}">		
					</form>
