...
<!--::header part start::-->
   <header class="main_menu home_menu">
	...
	<section>
			...
			<div style="position:absolute; left: 300px; top: 300px;z-index: 2;">
				<p id="oti">Charity Auctions
				<p>
				
			</div>
			<h1 style="font-family: ITC Grouch; text-align: center">With Charity Auctions, You can contribute to society!</h1><br><br>
	...
	<!-- breadcrumb start-->
		...
		<form id="form" action="${path}/front?key=auction&methodName=searchMinMax" method="post" style="width:-10px;float:left;">
			<input type="text" placeholder="추정가 최소값" name="min"/>
			<input type="text" placeholder="추정가 최대값" name="max"/>  
			<input type="submit" id="modify" value="검색" />
			<input type="hidden" name="aucCategory" value="2">
		</form>
		</div>
		...
		<form action="${path}/front?key=auction&methodName=searchByAuction" name="search" id="search" method="post" style="width:-10px;float:left;">		
				...
				<select name="keyField">
					<option value="artistName">작가명</option>
					<option value="artName">작품명</option>
					<option value="artCode">작품번호</option>
				</select>
		...
		<div id="c">
		<form action="${path}/front?key=auction&methodName=selectAuction&aucCategory=2" name="search" id="search" method="post" style="width:-10px;float:left;">
			<input type="submit" id="delete" value="전체검색">
		</form>		
		</div>
							...
							<div class="col-lg-4 col-sm-6">
								<div class="single_product_item">
									...
									<div class="single_product_text">
										<h4>작가명 : ${artDto.artistName}</h4>
										<h3>작품번호 : ${artDto.artCode}</h3>
										<h3>작품명 : ${artDto.artName}</h3>
										<h3>제작년도 : ${artDto.prdYear}</h3>
										<h3>
											추정가 : <fmt:formatNumber value="${artDto.estimateCost}" />
										</h3>
										<a
											href="${path}/front?key=auction&methodName=selectDetailArt&artCode=${artDto.artCode}" class="add_cart">상세보기<iclass="ti-heart">
											</i></a>
									</div>
								</div>
							</div>
