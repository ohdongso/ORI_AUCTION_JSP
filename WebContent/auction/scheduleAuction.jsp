...
<!--::header part start::-->
    ...
    <section>
			...
			<div style="position:absolute; left: 230px; top: 300px;z-index: 2;">
				<p id="oti">Scheduled Auctions
				<p>
				
			</div>
			<h1 style="font-family: ITC Grouch; text-align: center">All Upcoming Auctions </h1><br><br>
	...
	<div class="whole-wrap">
						...
						<table class="table table-borderless">
							<thead>
								<tr>
									...
									<th scope="col" style="color: red">&nbsp; 경매개시</th>
								</tr>
							</thead>
							<tbody>
								<c:choose>
								<c:when test="${empty requestScope.auctionDtoList}">
									<tr>
										<td colspan="12">
											<p align="center">
												<b><span style="font-size: 9pt;">예정중인 경매가 없습니다.</span></b>
											</p>
										</td>
									</tr>
								</c:when>
									...
									<c:forEach items="${requestScope.auctionDtoList}" var="Auction">	
										<tr onmouseover="this.style.background='white'"
											...
											<td bgcolor="">
												<p>
													...
													<form action="${path}/front?key=auction&methodName=updateForm" method="post">
														<input type="submit" value="수정" class="genric-btn primary">
											...
											<td bgcolor="">
												<p>
													...
													<form action="${path}/front?key=auction&methodName=deleteAuction" method="post">
														<input type="submit" id="AuctionDelete" value="삭제" class="genric-btn primary">
														<input type="hidden" name="aucCode" value="${Auction.aucCode}">
														<input type="hidden" name="aucState" value="${Auction.aucState}">
														<input type="hidden" name="artCode" value="${Auction.art.artCode}">
													</form>
											...
											<td bgcolor="">
												<p>
													<span style="font-size: 9pt;">
													<form action="${path}/front?key=auction&methodName=startAuction" method="post">
														<input type="submit" value="경매개시" id="startAuction" class="genric-btn primary">
														<input type="hidden" name="aucCode" value="${Auction.aucCode}">
		...
		<c:forEach items="${requestScope.auctionDtoList}" var="Auction">
			...
			<tr>
				...
				<td>${Auction.art.estimateCost}</td>
			...
			<td>
				...
				<form action="${path}/front?key=auction&methodName=startAuction" method="post">
					<input type="submit" value="경매개시" id="startAuction" style="width:-10px;float:left;">
					<input type="hidden" name="aucCode" value="${Auction.aucCode}">
					<input type="hidden" name="state" value="${Auction.aucState}">	
				</form>
