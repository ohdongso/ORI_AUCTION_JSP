...
<body>
	...
	<section>
		<div style="position: relative; z-index: 1; margin-bottom: 80px">
			<img src="${path}/consignment/img/bg2.png" width="100%" alt=""
				class="img-fluid" style="margin-top: 80px" id="mainI">
		</div>
		<div
			style="position: absolute; left: 300px; top:230px;  z-index: 2;">
			<p id="oti" style="color: white">Consignment List</p>
		</div>
	...
	<div class="col-lg-12">
		<div
			...
			<div class="single_product_menu d-flex">
				<p id="this">
					총 <span>${fn:length(conList)} </span> 개의 위탁신청
				</p>
			</div>
		</div>
	</div>
		...
		<div class="progress-table-wrap" style="overflow: hidden">
			<div class="progress-table">
				<table class="table table-borderless">
					...
					<c:choose>
						<c:when test="${empty requestScope.conList}">
							<tr>
								<td colspan="4">
									<p align="center">
										<b><span
											style="font-size: 9pt; font-family: adobe-caslon-pro, serif; font-weight: bold">등록된
												게시물이 없습니다.</span></b>
									</p>
								</td>
							</tr>
						...
						<c:otherwise>
							<c:forEach items="${requestScope.conList}" var="con">
								<tr>
									<td align="center">${con.consignCode}</td>
									<td align="center">${con.artCode}</td>
									<td align="center">${con.consignRdate}</td>
									<c:if test="${con.consignState==1}">
										<td align="center" style="color: red">In progress</td>
									</c:if>
									<c:if test="${con.consignState==2}">
										<td align="center" style="color: blue">Completed</td>
									</c:if>
									<td align="center">
										<form action="${path}/front?key=cons&methodName=selectByCode"
											method="post">
											<input type="hidden" name="consignCode"
												value="${con.consignCode}">
											<button type="submit"
												style="font-family: ITC Grouch; background-color: yellow; border: 0; outline: 0; padding: 10px 30px; font-size: 15px;">Detail</button>
										</form>
									</td>
								</tr>
							</c:forEach>
	...
	<c:if test="${sessionScope.loginState ne 0}">
		<div class="col-lg-12" style="text-align: right">
			<a href="${path}/front?key=cons&methodName=insertForm"
				class="genric-btn info" id="apply">Apply</a>
		</div>
	</c:if>
